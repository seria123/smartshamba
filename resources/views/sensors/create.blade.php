...
@section('title', 'Create Sensor - SmartShamba')

@section('content')
<div class="space-y-6">
    <!-- Page Header -->
    <div class="flex items-center justify-between">
        <h1 class="text-3xl font-bold text-gray-800">Create Sensor</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" 
                        ...
                        <option value="soil_ph" {{ old('type') == 'soil_ph' ? 'selected' : '' }}>Soil pH 🧪</option>
                        <option value="temperature" {{ old('type') == 'temperature' ? 'selected' : '' }}>Temperature 🌡️</option>
                        <option value="humidity" {{ old('type') == 'humidity' ? 'selected' : '' }}>Humidity 💨</option>
                        <option value="light_intensity" {{ old('type') == 'light_intensity' ? 'selected' : '' }}>Light Intensity ☀️</option>
                        <option value="rain_detection" {{ old('type') == 'rain_detection' ? 'selected' : '' }}>Rain Detection 🌧️</option>
                        <option value="multi" {{ old('type') == 'multi' ? 'selected' : '' }}>Multi-sensor</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
