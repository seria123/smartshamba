...
<div class="space-y-1">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('livestock_vaccination_schedules.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="animal_weight" id="animal_weight" value="{{ old('animal_weight') }}" step="0.01" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                </div>
                ...
                <div>
                    ...
                    <select name="route" id="route" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                        ...
                        <option value="IM" {{ old('route') == 'IM' ? 'selected' : '' }}>Intramuscular (IM)</option>
                        <option value="SC" {{ old('route') == 'SC' ? 'selected' : '' }}>Subcutaneous (SC)</option>
                        ...
                        <option value="nasal" {{ old('route') == 'nasal' ? 'selected' : '' }}>Nasal</option>
                    </select>
                </div>

                <div>
                    <label for="dose_amount" class="block text-sm font-medium text-gray-700 mb-2">Dose Amount</label>
                    <input type="number" name="dose_amount" id="dose_amount" value="{{ old('dose_amount') }}" step="0.01" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                </div>

                <div>
                    <label for="dose_unit" class="block text-sm font-medium text-gray-700 mb-2">Dose Unit</label>
                    <input type="text" name="dose_unit" id="dose_unit" value="{{ old('dose_unit') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary" placeholder="e.g., ml, mg">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="next_due_date" id="next_due_date" value="{{ old('next_due_date') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                </div>
            ...
            <div class="mt-6">
                ...
                <input type="date" name="administered_date" id="administered_date" value="{{ old('administered_date') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
            </div>
            ...
            <div class="mt-6">
                ...
                <input type="text" name="qr_code_data" id="qr_code_data" value="{{ old('qr_code_data') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary" placeholder="Scan or enter QR/barcode data">
            </div>

            <div class="mt-6">
                <label for="vet_notes" class="block text-sm font-medium text-gray-700 mb-2">Vet Notes</label>
                <textarea name="vet_notes" id="vet_notes" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">{{ old('vet_notes') }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    <label for="location" class="block text-sm font-medium text-gray-700 mb-2">Location (for Disease Trends)</label>
                    <input type="text" name="location" id="location" value="{{ old('location') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary" placeholder="e.g., Nairobi, Kisumu">
                </div>
                ...
                <div>
                    ...
                    <select name="season" id="season" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                        ...
                        <option value="rainy" {{ old('season') == 'rainy' ? 'selected' : '' }}>Rainy Season</option>
            ...
            <div class="mt-6 flex items-center">
                <div class="flex items-center">
                    <input type="checkbox" name="is_bulk_entry" id="is_bulk_entry" value="1" {{ old('is_bulk_entry') ? 'checked' : '' }} class="h-4 w-4 text-primary focus:ring-primary">
                </div>
            ...
            <div class="mt-6">
                ...
                <select name="user_role" id="user_role" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                    <option value="worker" {{ old('user_role') == 'worker' ? 'selected' : '' }}>Worker (Data Entry)</option>
                    <option value="vet" {{ old('user_role') == 'vet' ? 'selected' : '' }}>Vet (Can Approve)</option>
                    <option value="admin" {{ old('user_role') == 'admin' ? 'selected' : '' }}>Admin/Farmer (Full Access)</option>
                </select>
            </div>
