...
<div class="space-y-1">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('pest_control_schedules.update', $pestControlSchedule->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="scheduled_date" id="scheduled_date" value="{{ old('scheduled_date', $pestControlSchedule->scheduled_date?->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                </div>

                <div>
                    <label for="inspected_date" class="block text-sm font-medium text-gray-700 mb-2">Inspected Date</label>
                    <input type="date" name="inspected_date" id="inspected_date" value="{{ old('inspected_date', $pestControlSchedule->inspected_date?->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                </div>

                <div>
                    <label for="treatment_date" class="block text-sm font-medium text-gray-700 mb-2">Treatment Date</label>
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">{{ old('description', $pestControlSchedule->description) }}</textarea>
            </div>

            <div class="mt-6">
                <label for="treatment_method" class="block text-sm font-medium text-gray-700 mb-2">Treatment Method</label>
                <input type="text" name="treatment_method" id="treatment_method" value="{{ old('treatment_method', $pestControlSchedule->treatment_method) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
            </div>

            <div class="mt-6">
                <label for="treatment_notes" class="block text-sm font-medium text-gray-700 mb-2">Treatment Notes</label>
                <textarea name="treatment_notes" id="treatment_notes" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">{{ old('treatment_notes', $pestControlSchedule->treatment_notes) }}</textarea>
            </div>
